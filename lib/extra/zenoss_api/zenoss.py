...
class Zenoss(object):
    ...
    def get_devices_detailed(self):
        for d in self.get_devices()['devices']:
            yield self.__router_request('DeviceRouter', 'getInfo', data=[{'uid': d['uid']}])

    def get_device_detailed(self, uid):
        return self.__router_request('DeviceRouter', 'getInfo', data=[{'uid': uid}])
    ...
    def find_device_by_ip(self, ip):
        ...
        '''
        log.info('Finding device %s', ip)
        all_devices = self.get_devices()

        try:
            device = [d for d in all_devices['devices'] if d['ipAddressString'] == ip][0]
            # We need to save the has for later operations
            device['hash'] = all_devices['hash']
            log.info('%s found', ip)
            return device
        except IndexError:
            log.error('Cannot locate device %s', ip)
            raise Exception('Cannot locate device %s' % ip)
    ...
    """def add_device(self, device_name, ip, device_class, collector='localhost'):
        '''Add a device.

        '''
        log.info('Adding %s', device_name)
        data = dict(deviceName=device_name, deviceClass=device_class, manageIp=ip,
                    model=False, collector=collector)
        return self.__router_request('DeviceRouter', 'addDevice', [data])"""

    def add_device(self, device_name, ip, device_class, collector='localhost',
                   productionState=500, priority=30, locationPath='/Uknown'):
        '''Add a device.

        '''
        log.info('Adding %s', device_name)
        data = dict(deviceName=device_name, deviceClass=device_class, manageIp=ip,
                    model=False, collector=collector, productionState=productionState,
                    priority=priority, locationPath=locationPath)
        result = self.__router_request('DeviceRouter', 'addDevice', [data])
        log.debug(result)
        return result
    ...
    def remove_device(self, ip):
        ...
        '''
        log.info('Removing %s', ip)
        device = self.find_device_by_ip(ip)
    ...
    def remove_device_from_managed_group_by_ip(self, ip, group):
        '''Remove a device.

        '''
        log.info('Removing %s', ip, )
        device = self.find_device_by_ip(ip)
        data = dict(uids=[device['uid']], hashcheck=device['hash'], action='remove',
                    uid='/zport/dmd/Groups/Managed/' + group)
        return self.__router_request('DeviceRouter', 'removeDevices', [data])

    def remove_device_from_managed_system_by_ip(self, ip, system):
        '''Remove a device.
        ...
        device = self.find_device_by_ip(ip)
        data = dict(uids=[device['uid']], hashcheck=device['hash'], action='remove',
                    uid='/zport/dmd/Systems/Managed/' + system)
        return self.__router_request('DeviceRouter', 'removeDevices', [data])

    def move_device(self, ip, organizer):
        '''Move the device the organizer specified.

        '''
        log.info('Moving %s to %s', ip, organizer)
        device = self.find_device_by_ip(ip)
        data = dict(uids=[device['uid']], hashcheck=device['hash'], target=organizer, asynchronous=True)
        return self.__router_request('DeviceRouter', 'moveDevices', [data])

    def set_prod_state(self, ip, prod_state):
        '''Set the production state of a device.

        '''
        log.info('Setting prodState on %s to %s', ip, str(prod_state))
        device = self.find_device_by_ip(ip)
    ...
    def set_priority(self, ip, priority):
        '''Set the production state of a device.

        '''
        log.info('Setting prodState on %s to %s', ip, str(priority))
        device = self.find_device_by_ip(ip)
        data = dict(uids=[device['uid']], priority=priority, hashcheck=device['hash'])
        return self.__router_request('DeviceRouter', 'setPriority', [data])
    ...
    def rename_device(self, ip, new_name):
        ...
        '''
        data = dict(uid=self.find_device_by_ip(ip)['uid'], newId=new_name)
    ...
    def add_systems(self, system, description="", path=""):
        '''
        add group
        ...
        usage::
            ...
            u'success': True
        '''
        log.info('Adding Systems %s', system)
        base_org = "/zport/dmd/Systems/%s" % path
        data = dict(type='organizer', contextUid=base_org, id=system, description=description)
        return self.__router_request('DeviceRouter', 'addNode', [data])
    ...
    def add_location(self, parent_path="", name=""):
        '''
        Add Location
        ...
        name = name.strip('/')
        base_org = "/zport/dmd/Locations/%s" % parent_path
        data = dict(type='organizer', contextUid=base_org, id=name)
    ...
    def get_systems(self, groups='/zport/dmd/Systems', limit=None):
        '''
        get details of infrastructure group
        '''
        return self.__router_request('DeviceRouter', 'getSystems',
                                     data=[{'uid': groups, 'params': {}, 'limit': limit}])
